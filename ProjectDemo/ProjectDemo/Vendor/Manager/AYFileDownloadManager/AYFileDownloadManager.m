...
@interface AYFileDownloadManager() <NSCopying, NSURLSessionDelegate>
...
@implementation AYFileDownloadManager

static AYFileDownloadManager *_downloadManager;
 ...
 *  根据url获取对应的下载信息模型
 */
- (AYFileSessionModel *)getSessionModel:(NSUInteger)taskIdentifier
{
    return (AYFileSessionModel *)[self.sessionModels valueForKey:@(taskIdentifier).stringValue];
}
...
- (void)downloadWithURL:(NSString *)url attribute:(NSDictionary *)attribute fileName:(NSString *)fileName  progress:(void(^)(NSInteger receivedSize, NSInteger expectedSize, CGFloat progress))progressBlock state:(void(^)(DownloadState state))stateBlock
{
    ...
    AYFileSessionModel *sessionModel = [[AYFileSessionModel alloc] init];
...
- (void)deleteAllFile
{
    ...
    if ([fileManager fileExistsAtPath:CachesDirectory]) {
        ...
        for (AYFileSessionModel *sessionModel in [self.sessionModels allValues]) {
...
{
    AYFileSessionModel *sessionModel = [self getSessionModel:dataTask.taskIdentifier];
...
{
    AYFileSessionModel *sessionModel = [self getSessionModel:task.taskIdentifier];
